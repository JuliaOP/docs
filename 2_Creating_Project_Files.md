...
There is four sub-levels of the *model* level that are dedicated to represent the model relationships. All these relationships are optional:
...
At this moment, the "Belongs To Many" Relationships are limited and PWC is not generating the pivot table. Then, pleasespecify this table manually. We are working to solve this problem ASAP.
...
Name|Description|Obligatory?|Accepted Values
----|-----------|-----------|---------------
alias|Basically, the relationship name is the same as the related model. Eg: If we have a hasMany(orderItem), te relationship name will be "orderItem", and "orderItems" on plural. If you want to modify it, you can specify a relationship alias, eg: "item"|No, PWC generates using the name by default|String
...
validation|The relationship validation. Basically, for relationships, validation is ```required|integer``` for "Belongs To Reationships" (because of the Foreign Key)|No|Currently, the validation is made with the [Laravel Validation Syntax](https://laravel.com/docs/5.6/validation). PWC will separate the rules on Javascript Objects to be translated to other frameworks and languages
